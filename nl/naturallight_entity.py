...
class NLEntity(object):
    ...
    def __init__(self, fname):
        ...
        self.__map_data(isdfile)
        isdfile.close()

        datatable = self.get_dict_to_list(self.__data)
        self.__set_additional_data(datatable)
        self.__set_uv_dict(datatable)
        self.objDatetime = self.__parse_objdt(self.__general_information['Date'] + ' ' +
                                              self.__general_information['Time'])
    ...
    def get_objdt(self, tostr=False):
        if tostr:
    ...
    def get_dict_to_list(self, dict_src):
        retlist = []
    ...
    def get_ird(self, table, range_val1, range_val2, weight_func='none', alg='rect'):
    ...
    @staticmethod
    def get_specirrad_table(self, fname):
...
if __name__ == '__main__':
    entity = NLEntity('D:/_nldw/all/170515/FILE0001.ISD')
    print(entity.get_json('results', 'uv', 'general information'))
